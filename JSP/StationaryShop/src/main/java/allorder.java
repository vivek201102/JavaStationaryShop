...
public class allorder extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse res) throws IOException {
		...
		try {
			...
			out.print(
					"<body><nav class='navbar navbar-dark bg-primary'><div class='container-fluid'><a class='navbar-brand' href='#'>Stationary Shop</a><a class='navbar-brand navbar-right' href='index.jsp'>Log out</a></div></nav>");
			...
			String query = "select * from user where `mobile` = '" + mob + "'";
			ResultSet rs, rs2;
			int idc = -1;
			String email = null;
			...
			if (rs.next()) {
				...
				out.print(
						"<form class='container' style='max-width:70%;'><h4>Your Personal Details:</h4><table class='table table-striped'><tr><th>Name:</th><td>"
			...
			while (rs.next()) {
				...
				while (rs2.next()) {
					...
					s = "<tr><td>" + rs2.getString("oid") + "</td><td>" + rs2.getString("oname") + "</td><td>"
							+ String.valueOf(qty) + "</td><td>" + String.valueOf(rs2.getInt("totalprice") / qty)
							+ "</td></tr>";
					out.print(s);
				}
			...
			out.print("</form><div class='container' style='max-width:70%;'><button class='btn btn-primary' onclick='window.print()'>Print Orders</button>");
			out.print(
					"<br><br><button class='btn btn-primary' onclick='history.back()'>Go Back</button></body>");
		} catch (Exception e) {
